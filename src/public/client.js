...
const getRoverName = (state) => {
  let { apod } = state;

  fetch(`http://localhost:3000/getRoverName`)
    .then((res) => res.json())
    .then((apod) => updateStore(store, { apod }));
...
const getRoverCamerasAndPhotosInfo = (state) => {
  let { apod } = state;

  fetch(`http://localhost:3000/getRoverCamerasAndPhotosInfo`)
    .then((res) => res.json())
    .then((apod) => updateStore(store, { apod }));
